-- "Autores e Influenciadores" — permitir busca pelo nome da Entidade
-- .dev/specs/entities/author-linking.md
--
-- Pedido do usuário: "Em Autores e Influenciadores permitir busca pela
-- entidade." `AuthorRow.name` sempre foi o handle/nome bruto do autor
-- vindo da Brandwatch (`g.author`, ex: "revistaforum") — nunca o nome
-- oficial/formatado da Entity vinculada (`entities.name`, ex: "Revista
-- Fórum"). A guia "Por Entidade" já tinha um campo "Buscar", mas ele só
-- comparava contra esse handle bruto — buscar pelo nome como a Entity foi
-- cadastrada (com acento/espaçamento) não encontrava nada. `entity_name`
-- é aditivo, nunca substitui `name` (que continua sendo o identificador
-- de exibição principal da linha, ver AuthorsList).
...
comment on function get_authors_ranking(uuid, date, date, jsonb, text) is
  'Bloco `authors` do envelope. p_scope=''pautas'' (página /themes): escopa a Subcategories da Category raiz "Pautas" em vez da Query inteira/1 Narrativa, e agrupa por autor (um autor pode ter atividade em mais de uma pauta). narrative_labels = títulos das Narrativas/pautas em que o autor apareceu no escopo pedido, sempre array (nunca null). sentiment_* nunca lê bw_query_top_authors.sentiment_* (ver nota de 20260717000000). entity_id/entity_type/entity_name/entity_cargo/entity_partido/entity_ideologia/entity_influence_level/entity_tags: enriquecimento aditivo via LEFT JOIN entity_accounts (lower/trim username) -> entities (is_active=true) -> entity_tags, null/[] sem vínculo, nunca pré-requisito do ranking — ver .dev/specs/entities/author-linking.md. entity_name = entities.name (nome oficial/formatado da Entity cadastrada, ex: "Revista Fórum" — distinto de `name`, que é sempre o handle bruto vindo da Brandwatch, ex: "revistaforum"), adicionado para permitir busca pelo nome da Entity no frontend. mentions = soma de menções por autor (g.volume), sempre presente. followers = max(bw_query_top_authors/top_tweeters.followers) por autor (nunca somado entre categorias — atributo de perfil, não de atividade). platforms = bw_top_author_platform_tags(platform_stats) deduplicado por autor, sempre array (vazio quando nenhuma chave de plataforma conhecida está presente no jsonb sincronizado).';
