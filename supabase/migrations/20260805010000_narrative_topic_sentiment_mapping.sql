-- Pedido do usuário (2026-07-14): "Na integração com a brandwatch temos os
-- topics vinculados com as narrativas. Esses tópicos precisam aparecer no
-- envelope das narrativas para que a IA consiga utilizar em sua análise...
-- No caso das narrativas é importantíssimo esse mapeamento dos tópicos com
-- a narrativa para melhorar o entendimento da IA e do usuário final."
--
-- Até esta migration, `get_narratives_table.tags` já trazia os top 6
-- termos/hashtags por Narrativa (bw_query_topics), mas sem nenhuma
-- informação de polaridade — um mapeamento "topic → narrativa" existe,
-- mas não "topic positivo/negativo → narrativa", que é o que o usuário
-- pede explicitamente. `get_term_signals` já resolve isso, mas só quando
-- escopado a UMA Narrativa por vez (via filters.narratives) — não dá pra
-- usá-lo para preencher N linhas simultâneas da tabela de Narrativas
-- (overview/narrativas/pautas), nem para o payload da IA que resume UMA
-- Narrativa por vez sem uma segunda chamada RPC. Por isso o mapeamento
-- positivo/negativo entra como colunas adicionais desta function — mesma
-- classificação por maioria já usada por `get_term_signals`
-- (sentiment_positive/neutral/negative, o balde com mais menções vence),
-- só que calculada por Narrativa (category_id) em vez de por filtro
-- arbitrário.
--
-- `drop function` necessário: adiciona colunas de saída (mesma regra já
-- documentada em CLAUDE.md/sql-aggregation.md — `create or replace` não
-- deixa aumentar a lista de colunas de um `returns table`, ver
-- `20260721010000`/`20260725050000`).
...
as $$
  ...
  -- Mesma classificação por maioria já usada por `get_term_signals`
  -- (sentiment_positive/neutral/negative — o balde com mais menções
  -- vence), aplicada aqui por Narrativa (category_id) em vez de por
  -- filtro arbitrário. Uma única leitura de bw_query_topics alimenta os
  -- 3 rankings abaixo (tags geral/positivo/negativo).
  topic_classified as (
    select
      t.category_id,
      t.label,
...
comment on function get_narratives_table(uuid, date, date, jsonb, uuid, text, timestamptz) is
  'Bloco `narratives` do envelope. ÚNICO overload desta function (migration 20260731040000 consolidou dois overloads conflitantes). p_scope: null = sem filtro extra, ''roots'' = só Category de topo, ''leaves'' = só Subcategory de qualquer Category, ''pautas'' = só Subcategory cuja Category-pai é a Category raiz "Pautas". p_reference_at (default now()): ancora o cálculo de Tendência numa data específica — usado por get_communication_impact. risk_score = greatest(fórmula composta de sentimento+momentum+tendência+alcance+influência+impacto, MAX(feed_events.severity_score) entre eventos ativos da Narrativa) — event-radar (A2) só pode ELEVAR o risco mostrado, nunca derrubá-lo. net_sentiment/sentiment_label: quando o balde Neutro é predominante (>= positivo e >= negativo), o rótulo é sempre ''neutral'' e o score reportado é 0; só quando Neutro não é predominante o skew (positivo-negativo)/(positivo+negativo) decide o rótulo nas 7 faixas de sempre. category_label = nome da Category-pai de bw_categories. tags = top 6 termos/hashtags da Narrativa por volume, sem filtro de sentimento. positive_topics/negative_topics (2026-07-14): top 5 termos/hashtags cujo sentimento predominante (maioria dos 3 contadores de bw_query_topics) é positivo/negativo, mesma classificação de get_term_signals aplicada por Narrativa — mapeamento tópico↔Narrativa por polaridade, usado pelo card de Narrativa e pelo payload de narrative_summary_build_payload (IA).';

-- narrative_summary_build_payload (20260804010000) passa a incluir
-- positive_topics/negative_topics no mesmo objeto `scores` que já lê de
-- get_narratives_table — a IA que escreve narratives.description ganha
-- visibilidade de QUAIS termos específicos puxam o sentimento da
-- Narrativa pra cada lado, não só o `tags` geral (sem polaridade) que já
-- tinha antes. Mesma assinatura/retorno (jsonb) — sem `drop function`
-- necessário.
