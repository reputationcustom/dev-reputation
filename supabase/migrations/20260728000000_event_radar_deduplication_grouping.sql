-- event-radar 1.2 — deduplication-grouping (SQL, sem IA).
-- Implementa .dev/specs/event-radar/deduplication-grouping.md.
--
-- ⚠️ Metade do que este spec pede já estava implementada desde 1.1
-- (migration 20260727000000), não por engano: a chave de dedup "insere se
-- não existe, atualiza se já existe ativo" (deduplication-grouping.md,
-- itens 2-4, "Regras de negócio" — "a lógica de upsert deve se apoiar
-- nessa constraint") é exatamente o que o próprio `run_event_detection()`
-- já faz via `INSERT ... ON CONFLICT (...) WHERE closed_at IS NULL DO
-- UPDATE` sobre `radar_staging_events_active_key` — não existe uma
-- segunda camada de dedup a escrever aqui em cima disso. O que 1.1
-- genuinamente não fazia (e é a única peça nova desta migration): item 5
-- do spec — "quando o indicador que originou o evento volta ao normal
-- ... marca o evento como encerrado (closed_at)".
--
-- Essa peça é implementada dentro do próprio `run_event_detection()`
-- (CREATE OR REPLACE, não uma função/step separado): "voltou ao normal" só
-- é conhecível comparando contra a mesma varredura de regras que 1.1 já
-- calcula a cada ciclo — uma função de encerramento separada precisaria
-- recalcular a mesma matriz de agregações só para descobrir a mesma coisa
-- (custo de leitura dobrado, sem benefício). `v_cycle_start` (capturado
-- uma vez no início do ciclo, usado como valor de `detected_at` em todo
-- INSERT/UPDATE que antes usava `now()`) é o que permite, ao final,
-- identificar em uma única instrução quem NÃO foi tocado neste ciclo —
-- ou seja, qualquer linha ativa cuja regra deixou de disparar — e fechar
-- (`closed_at`) essas linhas.
