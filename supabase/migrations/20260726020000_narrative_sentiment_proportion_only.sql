-- Bug real reportado pelo usuário via screenshot (2026-07-25, mesmo dia da
-- migration 20260725000000, mas causa raiz diferente): o card "Economia"
-- mostrava pos 17.1% / neu 40.3% / neg 42.5% (negativo é o predominante),
-- mas a borda esquerda do card e a coluna "Sentimento" da tabela
-- "Todas as Narrativas" indicavam Neutro.
--
-- 20260725000000 já tinha corrigido uma causa real desta mesma classe de
-- sintoma (janela de tempo divergente: `sentiment_label` vinha de um único
-- dia, `sentiment_positive_pct`/etc. vinham do período somado). Essa causa
-- está corrigida — `sentiment_final` já lê `period_agg` para os dois. Mas
-- persistia uma SEGUNDA causa, agora encontrada: quando o `net_sentiment`
-- oficial da Brandwatch (`narrative_metrics.net_sentiment`, endpoint
-- `data/netSentiment/categories/days` — ver `foundation/data-model.md`)
-- está sincronizado para os dias do período, `sentiment_final` preferia a
-- média ponderada desse score sobre o cálculo local — mas `net_sentiment`
-- vem de uma chamada de API **diferente e independente** de
-- `data/volume/sentiment/days` (a origem de `sentiment_positive/neutral/
-- negative`, os mesmos 3 números exibidos na barra do card). São dois
-- agregados oficiais da Brandwatch, cada um calculado pelo motor deles sem
-- garantia de reconciliação entre si — o "Economia" é exatamente esse
-- caso: `net_sentiment` oficial caiu na faixa neutra (-4 a 5) enquanto a
-- proporção real entre as 3 categorias classificadas (a mesma que a barra
-- do card mostra) tem negativo como maioria. Resultado: o rótulo/borda
-- podiam contradizer visualmente os próprios números que o card exibe ao
-- lado — o mesmo tipo de contradição de "duas fontes divergentes no mesmo
-- card" que 20260725000000 já tinha fechado pra janela de tempo, agora
-- fechado pra fonte de dado.
--
-- Correção: `sentiment_label`/`net_sentiment` (bloco `narratives`) passam
-- a vir SEMPRE do cálculo local sobre `sentiment_positive_pct`/
-- `neutral_pct`/`negative_pct` (positivo-negativo)/(positivo+negativo)*100
-- — a mesma base de `period_agg.sentiment_positive_current`/
-- `neutral_current`/`negative_current` já usada pelas 3 porcentagens
-- exibidas na barra do card. Isso não é um cálculo local sobre `mentions`
-- amostrada (Princípio técnico 2) — `sentiment_positive/neutral/negative`
-- em `narrative_metrics` já são um agregado oficial da Brandwatch
-- (`data/volume/sentiment/days`), só reaproveitado numa fórmula derivada;
-- é a mesma lógica que já era usada como fallback desde 20260720000000,
-- só que agora é a ÚNICA fonte (não mais um fallback condicional ao
-- `net_sentiment` oficial não ter sincronizado) — garante por construção
-- que o rótulo/borda nunca podem discordar da barra pos/neu/neg do mesmo
-- card, já que vêm exatamente dos mesmos 3 números.
-- `risk_inputs.sentiment_risk` (que já lia `sentiment_labeled.net_sentiment`)
-- herda a correção automaticamente, sem mudança própria.
--
-- Mesma assinatura e mesmas colunas de saída de 20260725050000 — só o
-- corpo das CTEs `period_agg`/`sentiment_final`/`sentiment_labeled` muda,
-- `create or replace` basta (sem `drop function`).
...
as $$
  ...
  -- ✅ Fonte única do sentimento (2026-07-25, migration 20260725060000):
  -- sempre a proporção local sobre as mesmas somas de
  -- sentiment_positive/neutral/negative do período usadas por
  -- sentiment_positive_pct/neutral_pct/negative_pct — nunca mais a média
  -- ponderada do net_sentiment oficial (endpoint diferente de
  -- data/volume/sentiment/days, sem garantia de reconciliar com a barra
  -- pos/neu/neg do mesmo card, ver comentário no topo do arquivo).
  sentiment_final as (
    select
      narrative_id,
      case
...
comment on function get_narratives_table(uuid, date, date, jsonb, uuid, text) is
  'Bloco `narratives` do envelope. p_scope: null = sem filtro extra, ''roots'' = só Category de topo, ''leaves'' = só Subcategory de qualquer Category, ''pautas'' = só Subcategory cuja Category-pai é a Category raiz "Pautas" (pautas_root_category_id). risk_score tem um termo de interação (migration 20260725000000): a contribuição conjunta de momentum_score+trend_score é amortecida (piso 50%) quando sentiment_risk é baixo (sentimento líquido positivo). net_sentiment/sentiment_label (correção 20260725060000) vêm SEMPRE do cálculo local sobre sentiment_positive_pct/neutral_pct/negative_pct do período — nunca da média do net_sentiment oficial da Brandwatch (endpoint diferente, sem garantia de reconciliar com a barra pos/neu/neg do mesmo card, causa raiz de um bug real reportado pelo usuário). category_label (migration 20260725050000) = nome da Category-pai de bw_categories; para uma linha de escopo ''roots'' (sem pai), cai no próprio nome da Category.';
