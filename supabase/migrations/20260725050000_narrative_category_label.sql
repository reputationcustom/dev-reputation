-- Pedido do usuário (2026-07-25): "Na página de Narrativas os cards que
-- ficam abaixo, devem ser organizados pela categoria... utilizar raia ou
-- outro componente que achar mais apropriado para facilitar o agrupamento
-- e localização da narrativa."
--
-- `get_narratives_table` (`/narratives`, escopo `p_scope => 'leaves'`, ver
-- CLAUDE.md "Narrative naming/scope change") só devolve Narrativas-folha
-- (Subcategory) desde 2026-07-21 — o nome da Category-pai (a Pauta/tema a
-- que a Subcategory pertence) nunca foi exposto no envelope, então o
-- frontend não tinha como agrupar os cards por categoria sem inventar uma
-- segunda chamada. `bw_categories.parent_id` já resolve essa relação
-- (mesma tabela/coluna usada por `pautas_root_category_id()` e pelo
-- escopo `'leaves'`/`'pautas'` desta própria function) — não é um dado
-- novo, só nunca tinha sido devolvido nesta function.
--
-- Novo campo `category_label`: nome da Category-pai da Subcategory
-- (`bw_categories.parent_id`). Para uma linha de escopo `'roots'` (Category
-- de topo, sem pai — usada por outras páginas, não por `/narratives`),
-- cai no próprio nome da Category, já que "a categoria dela" é ela mesma
-- nesse caso — nunca `null`, sempre um agrupador válido pro frontend.
--
-- Precisa dropar a function (não só `create or replace`) porque estamos
-- adicionando coluna de saída — mesma trava do Postgres já documentada em
-- 20260717000000/20260721010000 (`cannot change return type of existing
-- function`).
...
as $$
  ...
  -- Sentimento consistente com o período agregado (não o snapshot de um
  -- único dia de `latest_day`) — ver migration 20260725000000.
  sentiment_final as (
    select
      narrative_id,
      coalesce(
...
comment on function get_narratives_table(uuid, date, date, jsonb, uuid, text) is
  'Bloco `narratives` do envelope. p_scope: null = sem filtro extra, ''roots'' = só Category de topo, ''leaves'' = só Subcategory de qualquer Category, ''pautas'' = só Subcategory cuja Category-pai é a Category raiz "Pautas" (pautas_root_category_id). risk_score ganhou um termo de interação (migration 20260725000000): a contribuição conjunta de momentum_score+trend_score é amortecida (piso 50%) quando sentiment_risk é baixo (sentimento líquido positivo). net_sentiment/sentiment_label são calculados sobre o MESMO período agregado de sentiment_positive_pct/neutral_pct/negative_pct (correção 20260725000000). category_label (migration 20260725050000) = nome da Category-pai de bw_categories (a Pauta/tema a que a Subcategory pertence); para uma linha de escopo ''roots'' (sem pai), cai no próprio nome da Category — usado pelo frontend pra agrupar os cards da lista de Narrativas por categoria.';
