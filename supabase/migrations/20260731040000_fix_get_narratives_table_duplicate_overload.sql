-- Bug real encontrado (2026-07-14): `/narratives`, `/overview` e
-- `get-narrative-detail` retornavam o bloco `narratives`/`ui_meta.narrative`
-- sempre vazio (`[]`/`null`), apesar de dado confirmado por SQL direto
-- (Narrativas-folha ativas + `narrative_metrics` reais na janela pedida).
-- Já se tinha descartado ausência de dado, RLS óbvio e cache de página
-- (`page_cache`, desabilitado numa sessão anterior enquanto se investigava
-- isso) — nenhum dos dois era a causa real.
--
-- Causa raiz: `get_narratives_table` existe hoje como DOIS overloads
-- conflitantes no banco, mesma classe de bug já documentada em CLAUDE.md
-- ("Narrative card redesign...", 2026-07-21) para esta mesma function:
--   1) `20260726010000` (`communications`) adicionou `p_reference_at
--      timestamptz default now()` (6 → 7 parâmetros) — corretamente fez
--      `drop function if exists get_narratives_table(uuid, date, date,
--      jsonb, uuid, text)` antes de recriar com 7 parâmetros.
--   2) `20260726020000` (mesma sessão, minutos depois — fix de sentimento
--      "proportion only") fez só `create or replace function
--      get_narratives_table(6 parâmetros)`, SEM `drop function` antes —
--      como a assinatura de 6 parâmetros tinha acabado de ser dropada por
--      (1), este `create or replace` não substitui nada, CRIA um novo
--      overload de 6 parâmetros, coexistindo com o de 7 parâmetros.
--
-- Resultado: dois `get_narratives_table` no catálogo, divergentes entre si
-- (o de 7 parâmetros nunca ganhou `category_label`, nem o fix de
-- sentimento "proportion only" — foi escrito a partir de uma cópia mais
-- antiga da function). Toda chamada via PostgREST/`supabase.rpc(...)`
-- (get-page-overview/get-page-narratives/get-narrative-detail, todas
-- passam exatamente 6 argumentos nomeados, nunca `p_reference_at`) fica
-- sujeita à resolução de overload do PostgREST — que, com dois candidatos
-- do mesmo nome, pode falhar a escolher um único ("could not choose the
-- best candidate function", `PGRST203`) em vez de simplesmente preferir o
-- de menos argumentos default como o Postgres faria numa chamada SQL pura.
-- Esse erro nunca chega ao usuário como tal: `fetchNarratives`/
-- `fetchNarrativeSummary` (aggregated-metrics-service.ts, e as 7 cópias
-- inline nas Edge Functions, Princípio técnico 5) capturam qualquer erro
-- da RPC e devolvem `[]`/`null` silenciosamente — daí o sintoma observado.
--
-- Fix: consolidar em UM ÚNICO `get_narratives_table`, 7 parâmetros
-- (mantém `p_reference_at`, usado por `get_communication_impact`/
-- `get_narrative_communication_timeline`), reunindo as duas metades que
-- haviam divergido: `category_label` + sentimento "proportion only" (de
-- `20260726020000`) e `p_reference_at` no cálculo de Tendência (de
-- `20260726010000`). `get_communication_impact` chama esta function com
-- argumentos nomeados incluindo `p_reference_at` explicitamente — resolve
-- sem ambiguidade contra um único overload, comportamento inalterado.
...
as $$
  ...
  -- Fonte única do sentimento (herdado de 20260726020000): sempre a
  -- proporção local sobre as mesmas somas usadas por
  -- sentiment_positive_pct/neutral_pct/negative_pct — nunca a média do
  -- net_sentiment oficial da Brandwatch (endpoint diferente e
  -- independente, sem garantia de reconciliar com a barra pos/neu/neg do
  -- mesmo card, causa raiz de um bug real já documentado em CLAUDE.md).
  sentiment_final as (
    select
      narrative_id,
      case
  ...
  -- Herdado de 20260726010000: ancora a janela de 14 dias da Tendência em
  -- p_reference_at (default now()) em vez de current_date hardcoded —
  -- permite get_communication_impact reconstruir Tendência "como estava"
  -- numa data histórica (antes/depois de uma Comunicação/Decisão), sem
  -- afetar nenhum consumidor de página (todos herdam o default).
  trend_series as (
    select
...
comment on function get_narratives_table(uuid, date, date, jsonb, uuid, text, timestamptz) is
  'Bloco `narratives` do envelope. ÚNICO overload desta function (migration 20260731040000 consolidou dois overloads conflitantes — 6 e 7 parâmetros — que coexistiam desde 20260726010000/20260726020000 e faziam PostgREST falhar a resolver a RPC pra get-page-overview/get-page-narratives/get-narrative-detail, causa raiz real de `/narratives` retornando vazio). p_scope: null = sem filtro extra, ''roots'' = só Category de topo, ''leaves'' = só Subcategory de qualquer Category, ''pautas'' = só Subcategory cuja Category-pai é a Category raiz "Pautas". p_reference_at (default now()): ancora o cálculo de Tendência (regressão de 14 dias) numa data específica — usado por get_communication_impact, nenhum consumidor de página passa esse parâmetro. risk_score tem um termo de interação: a contribuição conjunta de momentum_score+trend_score é amortecida (piso 50%) quando sentiment_risk é baixo. net_sentiment/sentiment_label vêm SEMPRE do cálculo local sobre sentiment_positive_pct/neutral_pct/negative_pct do período — nunca do net_sentiment oficial da Brandwatch. category_label = nome da Category-pai de bw_categories; para uma linha de escopo ''roots'' (sem pai), cai no próprio nome da Category.';
