// supabase/functions/delete-finops-manual-cost/index.ts
//
// Edge Function autossuficiente (Princípio técnico 5) — módulo `finops`.
// Remove um custo extra cadastrado. Admin-only, mesmo padrão de auth dos
// admin-*.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabaseAdmin.from("manual_costs").delete().eq("id", id);
    if (error) {
      console.error("[delete-finops-manual-cost] delete failed", error);
      return jsonResponse({ error: "Não foi possível excluir. Tente novamente." }, 500);
    }

    return jsonResponse({ ok: true });
  } catch (err) {
    console.error("[delete-finops-manual-cost] handler failed", err);
    return jsonResponse({ error: "Não foi possível excluir. Tente novamente." }, 500);
  }
});
