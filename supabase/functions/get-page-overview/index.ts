...
// tags/positive_topics/negative_topics (get_narratives_table) e
// term_signals (get_term_signals): 'trending' (default, mesmo default do
// lado SQL) ou 'volume'. Qualquer outro valor cai em 'trending'.
function normalizeTopicSort(raw?: string): 'trending' | 'volume' {
  return raw === 'volume' ? 'volume' : 'trending'
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      ...
      .maybeSingle()
    if (membershipError) {
      console.error('[get-page-overview] membership check failed', membershipError)
    ...
    const envelope = await getPageEnvelopeWithCache(supabase, 'overview', {
      organizationId,
      period,
      filters: normalizeFilters(body.filters),
      topicSort: normalizeTopicSort(body.topic_sort),
    })

    return jsonResponse(envelope, 200)
  } catch (err) {
    console.error('[get-page-overview] unhandled error', err)
