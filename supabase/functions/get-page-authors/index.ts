// Edge Function: get-page-authors
// Fonte: .dev/specs/aggregated-metrics/edge-functions-per-page.md
//
// Página "Autores e Influenciadores" (`/authors`) — ✅ implementada
// 2026-07-25 (pedido do usuário: mover "Perfis relevantes"/"X Themes" de
// `/platforms` pra cá). PAGE_BLOCKS.authors = ['authors', 'x_insights'] —
// mesmos 2 blocos que já alimentavam esses 2 widgets em `/platforms`,
// nenhuma function SQL nova (get_authors_ranking/get_x_insights já
// existiam). Primeira função nova desde a 6ª (`get-page-themes`).
//
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      ...
      .maybeSingle()
    if (membershipError) {
      console.error('[get-page-authors] membership check failed', membershipError)
    ...
    const envelope = await getPageEnvelopeWithCache(supabase, 'authors', {
      organizationId,
      period,
    ...
    console.error('[get-page-authors] unhandled error', err)
