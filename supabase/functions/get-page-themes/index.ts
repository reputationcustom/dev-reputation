...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: membership, error: membershipError } = await supabase
      ...
      .maybeSingle()
    if (membershipError) {
      console.error('[get-page-themes] membership check failed', membershipError)
    ...
    const envelope = await getPageEnvelopeWithCache(supabase, 'themes', {
      organizationId,
      period,
    ...
    console.error('[get-page-themes] unhandled error', err)
