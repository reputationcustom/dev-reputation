...
| 37 | `entities`/`intelligence-center` | Entity cadastrada manualmente ("Revista Fórum") reportada mostrando `ideologia = "Direita"` na UI apesar do usuário ter setado `esquerda` — auditoria de código completa (2026-08-09, `entity_match`/`ideologyLabel`/`ideologyBadgeClass`/`tailwind.config.ts`) não encontrou nenhum bug (todo mapeamento é por chave de dicionário direta, cores conferem com `IDEOLOGY_HEX`). Sem acesso ao banco real neste ambiente (RLS de `entities` exige `auth.role() = 'authenticated'`, a chave publicável sem sessão de login sempre volta vazia), não foi possível confirmar a causa. Hipótese mais plausível, já registrada como "Limitação aceita" em `entities/author-linking.md`: `get_authors_ranking`'s `entity_match` casa por `lower(trim(username))` **sem** filtrar por `platform` — se duas Entities diferentes têm uma conta com o mesmo texto de username em plataformas diferentes, a mais antiga (`order by ... created_at asc`) vence, mesmo sendo a errada. Próximo passo: usuário rodar `select id, name, ideologia from entities where name ilike '%fórum%' or name ilike '%forum%'` no SQL Editor do Supabase pra confirmar se há uma segunda Entity duplicada com esse handle (fix de dado) ou se o valor salvo realmente diverge do que o usuário pretendia (erro de digitação, também fix de dado, não de código). ✅ **`entity-registration.md` implementado (2026-07-15)** — `/admin/entities` agora existe, então esse diagnóstico/fix de dado pode ser feito direto pela UI (busca por nome, campo Ideologia do formulário) em vez de SQL direto; a causa em código (JOIN sem filtro por `platform`) continua a mesma, não fechada por essa UI | [entities/author-linking.md](entities/author-linking.md), [entities/entity-registration.md](entities/entity-registration.md) |
| 38 | `executive-reports` | Módulo novo, spec completa (2026-07-17) — `overview.md`/`data-model.md`/`executive-report.md`/`custom-report.md`, todos `status: pronto`. ✅ **Só o menu foi implementado (2026-07-17)**, a pedido explícito do usuário ("Altere apenas o menu... Não iremos desenvolver agora"): `sidebar.tsx` ganhou a seção "Relatórios" (abaixo de "Ações", 2 itens — Relatório Executivo/Personalizado), `/reports` virou um redirect de servidor pra `/reports/executive`, e as 2 rotas novas (`/reports/executive`/`/reports/custom`) renderizam `ComingSoonPage` ("em desenvolvimento"), mesmo padrão de `/alerts`/`/help`. **Nada mais foi implementado**: as 4 Edge Functions (`get-page-reports`, `get-page-reports-custom`, `export-report`, `list-generated-reports`), a migration de `reports_generated` + bucket de Storage `reports`, e as 3 extensões em `aggregated-metrics` (`get_volume_trend.p_grain_mode`, `PAGE_BLOCKS.reports`/novo page key `reports_custom`, Camada 2 `reports:executive_summary`) continuam pendentes na spec-fonte até o usuário pedir para desenvolver de verdade | [executive-reports/overview.md](executive-reports/overview.md) |
