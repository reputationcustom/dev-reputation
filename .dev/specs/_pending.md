...
|---|---|---|---|
| 7 | `aggregated-metrics` | Tabela `page_narrative_synthesis` (armazenamento persistente da síntese de página, Camada 1 de `ai-synthesis.md`) — spec pronta, sem migration. ⚠️ **Texto corrigido (2026-08-02)**: esta linha dizia "depende de `event-radar` publicar `feed_events`", desatualizado — esse pré-requisito já está satisfeito desde 2026-07-31 (ver gap #8, mesma correção). O que falta agora é só a implementação em si (tabela + lógica de composição), não mais uma dependência externa | [aggregated-metrics/ai-synthesis.md](aggregated-metrics/ai-synthesis.md) |
| 8 | `aggregated-metrics` | ⚠️ **Desbloqueado (2026-07-31), pré-requisito de leitura corrigido (2026-08-02)**: `get_active_highlights` (bloco `highlights`) depende de `feed_events`, que agora existe e está sendo populada por `event-radar` 1.4 (`agent-orchestrator`, migration `20260731020000`). A function SQL em si ainda não foi escrita — `fetchHighlights` na service layer continua retornando `[]` até essa function existir. Achado ao revisar `fluxo-aggregated-metrics.md` antes de liberar esta etapa: `feed_events.severity_score`/`severity` ficavam desatualizados enquanto o evento de origem seguisse ativo (só eram gravados uma vez, na publicação) — corrigido na migration `20260802000000` (espelhamento contínuo, mesmo padrão já usado pra `closed_at`), pré-condição real pro boost de `risk_score` (A2) funcionar com dado confiável quando implementado. Os outros 9 blocos do envelope já têm function SQL implementada (migration `20260714000000`) | [aggregated-metrics/sql-aggregation.md](aggregated-metrics/sql-aggregation.md), [event-radar/fluxo-aggregated-metrics.md](event-radar/fluxo-aggregated-metrics.md) |
...
| 33 | `event-radar` | ✅ **1.1-1.4/1.6 implementados (2026-07-27 a 2026-07-31)**, `data-model.md` completo (2026-08-01, migration `20260801000000`) — as 3 entidades (`radar_staging_events`/`feed_events`/`feed_event_feedback`) têm migration. Gap real restante: `schema-integration.md` continua `rascunho` porque **nenhuma UI existe** pra renderizar um card de `feed_events` nem pra dar feedback nele — depende de `get_active_highlights` (gap #8 abaixo), não pedido ainda. `aggregated-metrics-integration.md` também segue intocado | [event-radar/overview.md](event-radar/overview.md), [event-radar/schema-integration.md](event-radar/schema-integration.md) |
