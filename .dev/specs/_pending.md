...
| 8 | `aggregated-metrics` | ⚠️ **Desbloqueado (2026-07-31)**: `get_active_highlights` (bloco `highlights`) depende de `feed_events`, que agora existe e está sendo populada por `event-radar` 1.4 (`agent-orchestrator`, migration `20260731020000`). A function SQL em si ainda não foi escrita nesta sessão (fora do escopo pedido — só 1.4) — `fetchHighlights` na service layer continua retornando `[]` até essa function existir. Os outros 9 blocos do envelope já têm function SQL implementada (migration `20260714000000`) | [aggregated-metrics/sql-aggregation.md](aggregated-metrics/sql-aggregation.md), [event-radar/agent-orchestrator.md](event-radar/agent-orchestrator.md) |
...
| 33 | `event-radar` | ✅ **1.1 `detection-engine` (2026-07-27), 1.2 `deduplication-grouping` (2026-07-28), 1.3 `severity` (2026-07-29), 1.6 `volume-limits` (2026-07-30) e 1.4 `agent-orchestrator` (2026-07-31) implementados** (1.6 implementado antes de 1.4, ver nota de `overview.md`/"Ordem de implementação"). `feed_events` ganhou sua primeira migration junto com 1.4 (Claude Haiku 4.5, primeira Edge Function do módulo) — item 1 de `schema-integration.md` (escrita em `feed_events`) já está feito como parte do código de 1.4. Resto do módulo continua gap: `schema-integration.md` item 2 (`feed_event_feedback`, retroalimentação do analista — precisa de UI própria) — ainda `rascunho`, sem migration/código | [event-radar/overview.md](event-radar/overview.md), [event-radar/agent-orchestrator.md](event-radar/agent-orchestrator.md) |
