...
| `get_volume_trend(...)`                 | `trends`             | `bw_query_metrics_hourly`/`daily`/`weekly`/`monthly`, reaproveitando a regra de granularidade automática já especificada em `foundation/overview.md` ("Tabela interativa de Narrativas"/gráfico de volume) — não redefinir aqui. ✅ **Grão `hour` adicionado (2026-07-19)** — período de exatamente 1 dia (modo "Diário" do header) usa `bw_query_metrics_hourly`; `bucket_date` é `text`, não `date` (só o grão `hour` precisa de instante com hora, ver migration `20260719000000`). 📝 **`p_grain_mode` planejado (2026-07-17, spec do módulo `executive-reports`, ainda não implementado)** — novo parâmetro `text default 'auto_dashboard'` (`'auto_dashboard'` = comportamento atual, sem mudança para nenhum consumidor existente; `'auto_report'` = regra hora/dia/mês do Relatório Executivo, sem grão semanal — ver [executive-reports/data-model.md](../executive-reports/data-model.md)) |
