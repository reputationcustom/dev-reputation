...
| `get_theme_sov_trend(...)`              | `trends` (só `themes`)    | ✅ **Adicionado 2026-07-25** (migration `20260725030000`, gap #10) — `narrative_metrics` (Pautas) + `bw_query_metrics_daily` (denominador, Query inteira), um `group` por título de Pauta. SOV por bucket = soma de menções da Pauta / soma de menções da Query inteira nos dias daquele bucket, mesma definição de `narratives_overview.sov_percent` — `p_filters` mantido na assinatura por consistência, não usado (SOV nunca é filtrado por Narrativa). ✅ **Grão `hour` adicionado (2026-08-09)** — mesmo bug de `get_volume_trend` (2026-07-19): período de exatamente 1 dia ("Diário") caía no grão `day`, devolvendo 1 único ponto por Pauta (gráfico vazio na prática, reportado pelo usuário). Fonte: `bw_query_metrics_hourly` (mesma tabela de `get_volume_trend`, já tem `category_id`) — numerador via `category_id` da Pauta, denominador via `category_id is null` (Query inteira), mesmo `to_char(... at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS"Z"')` que o frontend (`TrendLineChart`) já distingue de um `bucket_date` diário pelo comprimento da string — nenhuma mudança de frontend precisou ser feita, mesma lógica genérica de `isHourlyPoint()` já cobre qualquer `Trend`/`series_by_group`, não só `get_volume_trend` |
